    ...
    <div className="flex-1 min-w-0 bg-white rounded-xl border border-[#E0E0E0] overflow-hidden">
      ...
      {items.map(item => (
        <div key={item.id} className="flex items-center border-b border-[#F5F5F5] hover:bg-[#FAFAFA] group px-3 py-1.5">
          ...
          <button onClick={() => onDelete(item.id)} className="opacity-0 group-hover:opacity-100 ml-1 w-4 h-4 rounded text-[#BDBDBD] hover:text-[#ee4266] transition text-xs flex items-center justify-center">×</button>
        </div>
      ))}
        ...
        {isAdding ? (
          <div className="flex gap-1">
            <input
              ...
              className="flex-1 min-w-0 px-2 py-0.5 text-xs rounded border border-[#D0D0D0] focus:outline-none focus:ring-1 focus:ring-[#035c37]"
            />
...
// ── Confetti burst ────────────────────────────────────────────────────────────
const CONFETTI_COLORS = ["#ffd23f","#0ead69","#ee4266","#035c37","#ee4266","#0ead69","#ee4266","#3bceac","#ffd23f","#0ead69"];
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-50 flex items-center justify-center overflow-hidden">
      ...
      <motion.div
        ...
        style={{ position: "absolute", width: 40, height: 40, borderRadius: "50%", backgroundColor: "#ffd23f", opacity: 0.5 }}
      />
    </div>
  );
}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-5 pb-10">
      ...
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => navigateMonth(-1)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-[#E8E8E8] text-[#035c37] transition">←</button>
        <h2 className="text-base font-semibold text-[#035c37]">{monthName}</h2>
        <button onClick={() => navigateMonth(1)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-[#E8E8E8] text-[#035c37] transition">→</button>
      </div>
      ...
      {editingGoal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center px-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
            <h3 className="font-semibold text-[#035c37] mb-4">Update savings goal</h3>
            <div className="relative mb-4">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#035c37] font-semibold">{symbol}</span>
              <input
                type="number" value={goalInput}
                onChange={e => setGoalInput(e.target.value)}
                className="w-full pl-8 pr-4 py-3 rounded-xl border border-[#D0D0D0] focus:outline-none focus:ring-2 focus:ring-[#035c37] text-[#035c37] text-lg font-semibold"
                autoFocus
              />
            </div>
            <div className="flex gap-3">
              ...
              <button onClick={updateGoal} className="flex-1 py-2.5 bg-[#035c37] text-white font-semibold rounded-xl hover:bg-[#035c37] transition">Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      <AnimatePresence mode="wait">
        {goalMet ? (
          <motion.div
            ...
            className="mb-5 bg-gradient-to-r from-[#e2f9f0] to-[#e0f9f4] border border-[#0ead69] rounded-xl px-5 py-3.5 flex items-center justify-between"
          >
            <div>
              <div className="text-sm font-bold text-[#035c37]">🎉 Goal smashed! Your tree grew this month.</div>
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="mb-5 bg-[#fff9d0] border border-[#ffd23f] rounded-xl px-5 py-3.5 flex items-center justify-between"
          >
            <div>
              <div className="text-sm font-semibold text-[#c23354]">
                ⏰ {daysLeft} day{daysLeft !== 1 ? "s" : ""} left — {fmt(savingsGoal - saved)} short of your goal
              </div>
              <div className="text-xs text-[#ee4266] mt-0.5">
        ...
        <div className="flex-1 min-w-0 space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {copyError && (
                <span className="text-xs text-[#ee4266]">{copyError}</span>
              )}
              <button
                ...
                className={`flex items-center gap-1.5 text-xs border rounded-lg px-2.5 py-1.5 transition font-medium ${
                  copyDone
                    ? "border-[#0ead69] text-[#035c37] bg-[#e2f9f0]"
                    : "border-[#D0D0D0] text-[#035c37] hover:border-[#035c37] hover:bg-[#F5F5F5] bg-white"
                }`}
              >
                {copying ? (
                  <div className="w-3 h-3 rounded-full border border-[#035c37] border-t-transparent animate-spin" />
                ) : copyDone ? (
                  <span>✓</span>
                ) : (
                  <span>↩</span>
                  ...
                  {items.map(item => isDual ? (
                    ...
                    <div key={item.id} className="grid grid-cols-[1fr_90px_90px_28px] border-b border-[#F5F5F5] hover:bg-[#FAFAFA] group">
                      ...
                      <div className="flex items-center justify-center">
                        <button onClick={() => deleteRow(item.id)} className="opacity-0 group-hover:opacity-100 w-4 h-4 rounded text-[#BDBDBD] hover:text-[#ee4266] transition text-xs flex items-center justify-center">×</button>
                      </div>
                    </div>
                  ) : (
                    /* ── SINGLE row ── */
                    ...
                    {isAdding ? (
                      <div className="flex gap-2 pl-9 pr-3 py-2">
                        <input
                          ...
                          placeholder="Item name…"
                          className="flex-1 px-2 py-1 text-sm rounded border border-[#D0D0D0] focus:outline-none focus:ring-1 focus:ring-[#035c37]"
                        />
        ...
        <div className="w-72 shrink-0 space-y-4 sticky top-20">
          ...
          <div className="bg-white rounded-xl border border-[#E0E0E0] p-4 shadow-sm">
            ...
            <div className="w-full bg-[#E8E8E8] rounded-full h-3 overflow-hidden mb-1">
              <div
                className={`h-3 rounded-full transition-all duration-500 ${saved >= 0 ? "bg-[#0ead69]" : "bg-[#ee4266]"}`}
              ...
              <ResponsiveContainer width="100%" height={90}>
                <BarChart data={ytdData} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                  ...
                  <Bar dataKey="saved" radius={[3,3,0,0]}>
                    {ytdData.map((p, i) => (
                      <Cell key={i} fill={p.isCurrent ? "#035c37" : "#0ead69"} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
