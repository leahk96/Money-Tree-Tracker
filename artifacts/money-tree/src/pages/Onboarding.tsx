    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#FAFAFA] px-4 py-12">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-[#035c37]">Let's set up your Money Tree</h1>
        </div>
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {STEPS.map((s, i) => (
            <div key={s.id} className="flex items-center gap-2">
              ...
              {i < STEPS.length - 1 && (
                <div className={`w-12 h-0.5 ${step > s.id ? "bg-[#0ead69]" : "bg-[#E8E8E8]"}`} />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[#E8E8E8] p-8">
          ...
          {step === 1 && (
            <div>
              <h2 className="text-xl font-semibold text-[#035c37] mb-2">What are you saving for?</h2>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-[#035c37] mb-1.5">Goal name</label>
                  <input
                    type="text"
                    value={goalName}
                    onChange={e => setGoalName(e.target.value)}
                    placeholder="e.g. Bali 2026, House deposit, Emergency fund"
                    className="w-full px-4 py-3 rounded-xl border border-[#D0D0D0] bg-[#FAFAFA] focus:outline-none focus:ring-2 focus:ring-[#035c37] text-[#035c37] placeholder-[#9E9E9E] transition"
                  />
                </div>

                <div>
                  <label className="block text-sm font-medium text-[#035c37] mb-1.5">
                  ...
                  {goalPhotoPreview ? (
                    <div className="relative">
                      ...
                      <button
                        ...
                        className="absolute top-2 right-2 w-7 h-7 bg-white rounded-full flex items-center justify-center text-[#546E7A] hover:text-[#ee4266] shadow-sm border border-[#e0e0e0] text-xs"
                      >
                        ✕
                      </button>
                    </div>
                  ) : (
                    <label className="flex flex-col items-center justify-center w-full h-32 rounded-xl border-2 border-dashed border-[#D0D0D0] bg-[#FAFAFA] cursor-pointer hover:border-[#035c37] hover:bg-[#F5F5F5] transition">
              ...
              <button
                onClick={handleStep1}
                className="w-full mt-6 py-3 bg-[#035c37] hover:bg-[#035c37] text-white font-semibold rounded-xl transition"
              >
                Continue
              </button>
            </div>
          ...
          {step === 2 && (
            <div className="text-center">
              ...
              <h2 className="text-xl font-semibold text-[#035c37] mb-2">You're all set!</h2>
              <p className="text-[#546E7A] text-sm mb-6">
                {goalName ? (
                  <>You're saving for <span className="font-semibold text-[#035c37]">{goalName}</span>. </>
                ) : null}
                Time to plant your Money Tree.
              </p>

              <div className="bg-[#F5F5F5] rounded-xl p-4 mb-6 text-left space-y-2">
                <div className="flex items-center gap-2 text-sm text-[#035c37]">
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => setStep(1)}
                  className="flex-1 py-3 border border-[#D0D0D0] text-[#035c37] font-medium rounded-xl hover:bg-[#F5F5F5] transition"
                >
                  Back
                </button>
                <button
                  ...
                  disabled={loading}
                  className="flex-1 py-3 bg-[#035c37] hover:bg-[#035c37] text-white font-semibold rounded-xl transition disabled:opacity-60"
                >
