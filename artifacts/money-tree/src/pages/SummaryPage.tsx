    ...
    <div className="max-w-2xl mx-auto px-4 py-6 pb-16 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <button
          ...
          disabled={!canPrev}
          className="w-9 h-9 rounded-full flex items-center justify-center hover:bg-[#E8E8E8] text-[#035c37] transition disabled:opacity-30"
        >
          ←
        </button>
        <div className="text-center">
          <h1 className="text-xl font-bold text-[#035c37]">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 space-y-2">
            ...
            <div className="w-full bg-[#E8E8E8] rounded-full h-2">
              <motion.div
                className="bg-[#035c37] h-2 rounded-full"
                initial={{ width: 0 }}
              ...
              >
                {earned ? (
                  <motion.div
                    ...
                    style={{ background: "linear-gradient(135deg,#ffd23f,#ee4266)" }}
                  >🪙</motion.div>
                ) : (
            ...
            return (
              <motion.button
                ...
                className={`rounded-xl p-2.5 text-left border-2 transition ${
                  isCurrentMonth ? "border-[#ffd23f] shadow-sm bg-white"
                  : d.goalMet ? "border-[#0ead69] bg-[#e2f9f0]"
                  : d.hasData ? "border-[#E8E8E8] bg-white"
                  : "border-dashed border-[#E8E8E8] bg-[#FAFAFA]"
                }`}
              >
                <div className={`text-xs font-semibold mb-1 ${d.goalMet ? "text-[#035c37]" : "text-[#546E7A]"}`}>
                  {MONTH_NAMES[i]}
                </div>
                {d.hasData ? (
                  <>
                    <div className={`text-sm font-bold tabular-nums ${d.goalMet ? "text-[#035c37]" : "text-[#035c37]"}`}>
                      {fmt(d.totalSaved)}
                    </div>
                    <div className={`text-[10px] mt-0.5 ${d.goalMet ? "text-[#0ead69]" : "text-[#ee4266]"}`}>
      ...
      <motion.button
        ...
        className="w-full py-3 bg-[#F5F5F5] border border-[#e2f9f0] rounded-xl text-sm font-medium text-[#035c37] hover:bg-[#e2f9f0] transition"
      >
        🌳 View your garden →
      </motion.button>
    </div>
